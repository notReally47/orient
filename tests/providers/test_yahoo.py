...
def test_a_midnight_timestamp_becomes_a_plain_date() -> None:
    ...
    provider: Final = YahooProvider(_fetch((_record(datetime(2026, 8, 12, 0, 0)),)))  # noqa: DTZ001  # the feed is naive
    assert provider.daily_bars("^GSPC", "5d")[0].session_date == date(2026, 8, 12)


def test_a_date_passes_through_unchanged() -> None:
    provider: Final = YahooProvider(_fetch((_record(date(2026, 8, 12)),)))
    assert provider.daily_bars("^GSPC", "5d")[0].session_date == date(2026, 8, 12)
...
def test_every_bar_is_returned_in_the_order_received() -> None:
    ...
    provider: Final = YahooProvider(_fetch(records))
