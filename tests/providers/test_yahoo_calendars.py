...
async def test_the_four_surfaces_flatten_into_one_list_tagged_by_kind() -> None:
    calendar: Final = await _calendars(
...
async def test_entries_sort_by_date_with_the_undated_ones_last() -> None:
...
async def test_only_the_kinds_asked_for_are_fetched() -> None:
    ...
    _ = await calendars.entries(START, END, ("earnings",))

    assert asked == ["earnings"]


async def test_a_row_yahoo_typed_unexpectedly_costs_itself_and_not_the_batch() -> None:
...
async def test_a_row_without_a_name_is_labelled_by_its_symbol() -> None:
    entry: Final = _only(
        await _calendars(earnings=({"symbol": "MSFT", "starts_at": date(2026, 8, 13), "timing": "TAS"},)).entries(
            START, END
        )
    )
...
async def test_the_window_asked_for_reaches_every_surface() -> None:
