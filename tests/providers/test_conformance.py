...
@_VENDORS
def test_bars_come_back_oldest_first(vendor: Vendor) -> None:
    """Every window calculation reads the last bar as the latest, so the order is the contract."""
    bars: Final = vendor.unordered_prices().daily_bars(SYMBOL, PERIOD)

    assert tuple(bar.session_date for bar in bars) == SESSIONS


@_VENDORS
def test_a_batched_fetch_orders_every_symbol_the_same_way(vendor: Vendor) -> None:
    """One symbol arriving reversed would give the backdrop yesterday's close for that one alone."""
    fetched: Final = vendor.unordered_prices().multi_bars((SYMBOL, OTHER), PERIOD)
...
@_VENDORS
def test_a_symbol_the_vendor_had_nothing_for_is_empty_rather_than_absent(vendor: Vendor) -> None:
    """A caller iterating its own basket must not get a KeyError because one fund did not price."""
    fetched: Final = vendor.one_symbol_prices().multi_bars((SYMBOL, OTHER), PERIOD)
...
@_VENDORS
def test_a_vendor_with_nothing_to_say_returns_empty_rather_than_raising(vendor: Vendor) -> None:
    prices: Final = vendor.silent_prices()

    assert prices.daily_bars(SYMBOL, PERIOD) == ()
    assert prices.multi_bars((SYMBOL,), PERIOD) == {SYMBOL: ()}


@_VENDORS
def test_calendar_entries_sort_by_date_with_the_undated_last(vendor: Vendor) -> None:
    """An event with no date still matters; sorting it first would bury what happens tomorrow."""
    calendar: Final = vendor.mixed_calendar().entries(WINDOW_START, WINDOW_END)

    assert tuple(entry.symbol for entry in calendar.entries) == (SOONER, LATER, UNDATED)


@_VENDORS
def test_a_calendar_says_how_much_it_could_not_read(vendor: Vendor) -> None:
...
@_VENDORS
def test_an_empty_week_is_an_empty_calendar_rather_than_a_failure(vendor: Vendor) -> None:
    calendar: Final = vendor.silent_calendar().entries(WINDOW_START, WINDOW_END)
...
@_VENDORS
def test_a_backdrop_with_nothing_to_report_yields_nulls_rather_than_raising(vendor: Vendor) -> None:
    """A dead upstream must degrade to a summary that says less, never to a run that says nothing."""
    context: Final = vendor.silent_market().backdrop()
...
@_VENDORS
def test_a_backdrop_with_nothing_to_report_still_counts_its_denominator(vendor: Vendor) -> None:
    """Zero advancers out of zero measured is a different statement from zero out of eleven."""
    breadth: Final = vendor.silent_market().backdrop().sector_breadth
