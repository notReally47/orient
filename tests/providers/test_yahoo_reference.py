...
def test_an_equity_profile_skips_the_fund_requests() -> None:
    profile: Final = _reference("EQUITY").profile("AAPL")
    assert profile.asset_class == "equity"
    assert profile.holdings == ()
    assert profile.sector_weights == {}


def test_a_fund_profile_carries_holdings_and_weights() -> None:
    profile: Final = _reference("ETF").profile("SPY")
...
def test_an_unknown_quote_type_leaves_the_asset_class_unset() -> None:
    """Guessing a class would send the writer to the wrong instrument skill."""
    assert _reference("SOMETHING_NEW").profile("???").asset_class is None


def test_the_implied_move_uses_the_strike_nearest_the_price() -> None:
    move: Final = _reference().implied_move("AAPL", 205.0, TODAY)
    assert move is not None
    assert move.implied_volatility == pytest.approx(0.20)


def test_the_implied_move_is_scaled_to_the_expiry_not_left_annual() -> None:
    """Seven days of a 20% annual volatility is well under 20%, and quoting 20% would be wrong."""
    move: Final = _reference().implied_move("AAPL", 200.0, TODAY)
    assert move is not None
    assert move.expiry == date.fromisoformat(EXPIRY)
    assert move.implied_move_percent == pytest.approx(0.20 * (7 / 365) ** 0.5)


def test_no_listed_expiry_means_no_figure_rather_than_a_zero() -> None:
    def none_listed(symbol: str) -> Sequence[str]:
        del symbol
        return ()

    reference: Final = YahooReference(_equity_info, _holdings, _weights, none_listed, _calls)
    assert reference.implied_move("AAPL", 200.0, TODAY) is None


def test_an_unpriced_chain_means_no_figure() -> None:
...
def test_a_nonsense_price_is_refused_rather_than_dividing_through() -> None:
    assert _reference().implied_move("AAPL", 0.0, TODAY) is None


def test_rating_actions_are_truncated_to_the_recent_ones() -> None:
    ...
    assert len(earnings.detail("AAPL", recent=5).recent_actions) == 5
